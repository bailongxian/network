...
static void BytesToBlock(unsigned char *buf,CBlowfish::SBlock *b);  
...
void CBlowfish::BlockToBytes(CBlowfish::SBlock *b, unsigned char *buf)  
{  
    unsigned int y;  
    unsigned char *p = buf;  
    /*Right*/  
...
void  CBlowfish::EncryptBlock(CBlowfish::SBlock *block)  
{  
    unsigned int uiLeft = block->m_uil;  
    unsigned int uiRight = block->m_uir;  
    uiLeft ^= m_auiP[0];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[1];  uiLeft ^= F(m_auiS, uiRight)^m_auiP[2];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[3];  uiLeft ^= F(m_auiS, uiRight)^m_auiP[4];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[5];  uiLeft ^= F(m_auiS, uiRight)^m_auiP[6];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[7];  uiLeft ^= F(m_auiS, uiRight)^m_auiP[8];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[9];  uiLeft ^= F(m_auiS, uiRight)^m_auiP[10];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[11]; uiLeft ^= F(m_auiS, uiRight)^m_auiP[12];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[13]; uiLeft ^= F(m_auiS, uiRight)^m_auiP[14];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[15]; uiLeft ^= F(m_auiS, uiRight)^m_auiP[16];  
    uiRight ^= m_auiP[17];  
    block->m_uil = uiRight;  
    block->m_uir = uiLeft;  
}
...
void CBlowfish::DecryptBlock(CBlowfish::SBlock *block)  
{  
    unsigned int uiLeft = block->m_uil;  
    unsigned int uiRight = block->m_uir;  
    uiLeft ^= m_auiP[17];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[16]; uiLeft ^= F(m_auiS, uiRight)^m_auiP[15];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[14]; uiLeft ^= F(m_auiS, uiRight)^m_auiP[13];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[12]; uiLeft ^= F(m_auiS, uiRight)^m_auiP[11];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[10]; uiLeft ^= F(m_auiS, uiRight)^m_auiP[9];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[8];  uiLeft ^= F(m_auiS, uiRight)^m_auiP[7];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[6];  uiLeft ^= F(m_auiS, uiRight)^m_auiP[5];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[4];  uiLeft ^= F(m_auiS, uiRight)^m_auiP[3];  
    uiRight ^= F(m_auiS, uiLeft)^m_auiP[2];  uiLeft ^= F(m_auiS, uiRight)^m_auiP[1];  
    uiRight ^= 	m_auiP[0];  
    block->m_uil = uiRight;  
    block->m_uir = uiLeft;  
}
...
int CBlowfish::EncryptByte(unsigned char *bin, size_t n, unsigned char *bout, int iMode)  
{  
    SBlock work;  
    unsigned char *in;  
    unsigned char *out;  
    ...
    {  
        SBlock chain; memcpy(&chain, &m_oChain, sizeof(SBlock));  
        for(; n >= 8; n -= 8, in += 8)  
        {  
...
int CBlowfish::DecryptByte(unsigned char *bin, size_t n, unsigned char *bout, int iMode)  
{  
    SBlock work;  
    unsigned char *in;  
    unsigned char *out;  
    ...
    if(iMode == CBC) /*CBC mode, using the Chain*/  
    {  
        SBlock crypt, chain; memcpy(&chain, &m_oChain, sizeof(SBlock));  
        for(; n >= 8; n -= 8, in += 8)  
        {  
            BytesToBlock(in, &work);  
            crypt = work;  
...
int CBlowfish::Init(unsigned char *ucKey, size_t keysize)  
{  
    ...
    for(i=0; i<MAX_PBLOCK_SIZE; )  
        EncryptBlock(&block), m_auiP[i++] = block.m_uil, m_auiP[i++] = block.m_uir;  
    for(j=0; j<MAX_SBLOCK_XSIZE; j++)  
        for(k=0; k<MAX_SBLOCK_YSIZE; )  
            EncryptBlock(&block), m_auiS[j][k++] = block.m_uil, m_auiS[j][k++] = block.m_uir;  
    return 1;  
}  
...
int CBlowfish::Encrypt(unsigned char* in, size_t siz_i, unsigned char* out, size_t siz_o, int iMode)  
{  
    ...
    nRet = EncryptByte(byte_in, nLen, byte_out, iMode);  
...
/*Decrypt from Input Buffer to Output Buffer 
  Returns false if n is multiple of 8*/  
int CBlowfish::Decrypt(unsigned char* in, size_t siz_i, unsigned char* out, size_t siz_o, int iMode)  
{  
    unsigned char *byte_in;  
    int nLen, nRet;  
    ...
    nRet = DecryptByte(byte_in, nLen, out, iMode);  
    free(byte_in);  
    return nRet;  
} 
